...
//   downloadsList.appendChild(progressContainer);
...
async function startDownload(format, downloadButton) {
  ...
  try {
    ...
    if (response.ok && data.success) {
      ...
      setTimeout(() => {
        if (downloadsContainer) {
          const containerTop = downloadsContainer.getBoundingClientRect().top + window.pageYOffset
          window.scrollTo({
            top: containerTop - 20,
            behavior: "smooth",
          })
...
function handleDownloadCompleted(taskId, task) {
  ...
  const progressFill = progressContainer.querySelector(".progress-fill")
  const messageDiv = progressContainer.querySelector(".download-message")
  const downloadActionsActive = progressContainer.querySelector(".download-actions-active")
  const downloadActionsCompleted = progressContainer.querySelector(".download-actions-completed")
  const downloadLink = progressContainer.querySelector(".btn-download")
  ...
  if (downloadLink && task.download_url) {
    downloadLink.href = task.download_url
    downloadLink.download = task.filename || "download"
    if (downloadActionsCompleted) downloadActionsCompleted.style.display = "block"
  }
...
function clearDownload(taskId) {
  ...
  if (downloadsContainer && downloadsList) {
    const remainingDownloads = downloadsList.querySelectorAll(".download-progress-item")
    if (remainingDownloads.length === 0) {
      downloadsContainer.style.display = "none"
    }
  }
...
function displayVideoInfo(data) {
  ...
  if (videoTitle) videoTitle.textContent = data.title || "YouTube Video"
  if (videoDuration) videoDuration.textContent = `⏱️ Duration: ${formatDuration(data.duration)}`
  if (videoUploader) videoUploader.textContent = `📺 Channel: ${data.uploader || "Unknown"}`
  if (videoViews) videoViews.textContent = `👁️ ${formatNumber(data.view_count)} views`
  if (videoLikes) videoLikes.textContent = `👍 ${formatNumber(data.like_count)} likes`
  if (uploadDate) uploadDate.textContent = `📅 ${formatDate(data.upload_date)}`
  if (videoDescription) videoDescription.textContent = data.description || "No description available"
