...
#         self.cancel_requests = set()  # Added cancel_requests set to track cancellations
...
#             # ✅ FIXED: Properly indented format processing code
...
#                 except Exception as e:
#                     if task_manager.is_cancelled(task_id) or "cancelled by user" in str(e).lower():
#                         return None
...
#         logger.info(f"Returning video info with video and audio formats")
        ...
        try:
            ...
            if has_720p:
                video_formats.append({
                    ...
                    'vcodec': 'h264',
                    'acodec': 'aac',
                    'recommended': is_long_video or not has_1080p,
...
async def shutdown_event():
    ...
    for task_id in active_task_ids:
        logger.info(f"Cancelling task {task_id} due to shutdown")
        task_manager.cancel_task(task_id)
    
    # Wait for active downloads to complete or timeout (max 30 seconds)
    shutdown_timeout = 30
    start_time = time.time()
    ...
    # Cleanup all remaining temp directories
    try:
        for task_id, file_info in list(file_manager._files.items()):
            file_manager.cleanup_file(task_id)
    except Exception as e:
